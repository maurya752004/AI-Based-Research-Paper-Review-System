...
### A. Source Code Samples

The following source code samples are short representative extracts from the implemented system. These are suitable for inclusion in an internship report because they show the overall flow of the project without adding the entire codebase.

#### A.1 Backend API Endpoint for Paper Review

This sample shows how the backend accepts a PDF, generates the review, stores the result, indexes it for RAG, and returns the response.
...
#### A.2 Core Review Generation Logic

This sample shows the main review flow. The system first tries local LLM review, then OpenAI review, and finally uses a fallback heuristic review for demo reliability.
...
def generate_review(text: str, title: str = "Untitled Paper") -> dict:
    ...
    if not _allow_fallback():
        raise RuntimeError(
            "AI review unavailable. Configure valid OPENAI_API_KEY/model access "
            "or set REVIEW_ALLOW_FALLBACK=true."
        )
...
#### A.3 RAG Indexing and Retrieval

This sample shows how the paper text is split into chunks and stored in ChromaDB, then retrieved later for question answering.
...
def index_submission(submission_id: int, title: str, full_text: str) -> int:
    ...
    metadatas = [
        {"submission_id": submission_id, "title": title, "chunk_index": i}
...
This sample shows the SQLite-based persistence layer used to store submissions, review jobs, audit events, and live paper feed data.
...
#### A.5 Frontend Upload and Analysis Flow

This sample shows how the Streamlit frontend uploads a PDF, calls the backend API, and displays the generated result.
...
with tab_single:
    ...
    if uploaded_file is not None and st.button("Analyze Paper", type="primary"):
        with st.spinner("Analyzing... this can take a few seconds"):
            review, submission_id, err = _post_review(
                uploaded_file.name,
...
#### A.6 Question Answering Over Indexed Paper Content

This sample shows how the system performs retrieval-based question answering over the indexed paper text.
...
### B. Sample Screens (to insert in report)

The following screenshots can be added in the appendix:

1. Login sidebar with role-based access
2. Single paper review result page
3. Compare papers output page
4. Research assistant Q&A page with citations
5. Ranking dashboard and analytics graphs
6. Human override submission form

### C. Reports / Outputs

The following generated outputs can also be attached in the appendix:

1. Sample AI review report PDF downloaded from the system
2. Submission history table
3. Ranking result table
4. Plagiarism check output
5. Multi-agent review output
6. Operational metrics summary

### D. Sample Environment Configuration
